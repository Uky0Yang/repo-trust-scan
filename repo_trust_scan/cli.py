...
def build_parser() -> argparse.ArgumentParser:
    ...
    scan.add_argument("--fail-on", choices=tuple(SEVERITY_ORDER), default="high", help="minimum severity that returns exit code 1")
...
def _normalize_default_scan(argv: list[str]) -> list[str]:
    if not argv:
        return ["scan"]
    if argv[0] in {"scan", "rules", "-h", "--help", "--version"}:
        return argv
    return ["scan", *argv]
...
def _text_report(report: ScanReport) -> str:
    ...
    lines = [
        ...
        f"risk={report.risk_score}/100 critical={counts['critical']} high={counts['high']} medium={counts['medium']} low={counts['low']} skipped={report.files_skipped}",
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = scan_repository(Path(args.path), max_file_bytes=args.max_file_bytes, ignored_rules=set(args.ignore))
